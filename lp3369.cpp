...
int zig(int i)
{
    ...
    if (f) {
        if (tree[f].son[LEFT] == f) {
            tree[f].son[LEFT] = tree[i].son[LEFT];
        } else {
            tree[f].son[RIGHT] = tree[i].son[LEFT];
        }
        tree[i].father = ls;
...
{
    node& n = tree[i];
    sprintf(info, "index: %d, value: %d, left: %d, right: %d, father: %d, count:"
                  "%d, subsize: %d",
        i, n.val, n.son[LEFT], n.son[RIGHT], n.father, n.count,
        n.subsize);
    return info;
}
